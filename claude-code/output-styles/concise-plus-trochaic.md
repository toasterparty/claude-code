...
Start each clause on a content word rather than on "the" or "a", and drop the article where the sentence survives without it. A catalectic line (nine syllables, ending on the stress) is allowed where the alternative is a filler word. Never pad with an inversion or an archaism to reach ten. An identifier or path counts as the syllables it is read aloud with; where one will not fit, let the verse line run long.
